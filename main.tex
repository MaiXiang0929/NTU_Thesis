...
\usepackage{xeCJK}                  % 解决中文与数学公式混排时的字体问题
...
\setCJKfamilyfont{songti}[
  AutoFakeBold = 3
]{SimSun}                           % 定义宋体字体族，启用自动加粗功能（AutoFakeBold）以确保在需要加粗时能正确显示
...
\titleformat{\section}
  ...
  {第\thesection 章\quad}    % 第X章
  {0pt} % 编号与标题之间的距离，小四号（12pt）字号下两个字符空格的长度为24pt
  {}

% 二级标题（节）格式：
\titleformat{\subsection}
  {\sihao\CJKfamily{songti}\bfseries} % 四号 (\sihao)、宋体加粗、居左
  {\thesubsection\quad}    % 1.1
  {0pt} % 编号与标题之间的距离，小四号（12pt）字号下两个字符空格的长度为24pt
  {}
...
% --- 公式按章编号，格式为 （1-1） ---
\numberwithin{equation}{section} % 将公式编号绑定到 section (章)
\renewcommand{\theequation}{\textnormal{\arabic{section}-\arabic{equation}}} % 格式化为：章-序号
\makeatletter
\def\tagform@#1{\maketag@@@{\xiaosi(\ignorespaces#1\unskip\@@italiccorr)}}
\makeatother
...
{\wuhao\songti 渲染管线\quad 材质Shader\quad 性能优化\quad HLSL算法} % 关键词（五号宋体）每一关键词后之间用二个字符空格分开，最后一个关键词后不打标点符号
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/1-1.png} % 图片文件名为“1-1.png”，请确保该图片文件与.tex文件在同一目录下，或者提供正确的路径
...
\begin{equation}
    % L, p, \omega 是变量（斜体）；max 是函数（正体）；数字是 Times New Roman
    L_o(p, \omega_o) = \int_{\Omega} f_r(p, \omega_i, \omega_o) L_i(p, \omega_i) \max(0, \bm{n} \cdot \omega_i) \, \mathrm{d}\omega_i
\end{equation}

% 带有单位和常量的示例
\begin{equation}
    % \exp 是预设函数（正体）；\pi 是常量（正体/斜体视标准而定，国内通常斜体，但 e 必须正体）
    \text{Color} = \text{Base} \times \exp( -k \cdot \text{distance} ) \quad (\text{Unit: } \mathrm{cd/m^2})
\end{equation}
